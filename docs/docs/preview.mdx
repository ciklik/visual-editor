...
When creating the editor you have to set up a `preview` attribute with an endpoint that will be called using the POST method for rendering the initial preview and every block on change
...
The server must return the HTML code of the component (without any extra HTML)
