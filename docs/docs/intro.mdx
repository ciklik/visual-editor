...
The editor uses custom element
...
```js
import { VisualEditor } from 'vvvvvvvv'

let editor = new VisualEditor()
```

Then register a new page component using the `registerComponent` method that expect 2 parameters
...
```js
import { HTMLText, Repeater, Text, Row, Select } from 'src/VisualEditor'
