...
export class Repeater extends AbstractField<FieldArgs, RepeaterLine[]> {
    ...
    return <div>
      ...
      <SortableWrapper items={value} onMove={handleMove}>
          <div class="ve-repeater">
            {value.map((line, k) => <this.fieldLine key={line._index} line={line} index={k}
  ...
  private fieldLine = ({
                         ...
                         onRemove,
                         onUpdate,
                       }: { line: RepeaterLine, index: number, onRemove: (line: RepeaterLine) => void, onUpdate: (path: string, v: unknown) => void, }) => {


    const handleRemove = () => onRemove(line)
    const handleUpdate = (path:string) => (value: unknown) => onUpdate(path, value)

    return <Sortable item={line} class="ve-repeater-item">
